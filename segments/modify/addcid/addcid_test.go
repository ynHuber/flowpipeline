...
func TestSegment_AddCid_noLocalAddrDrop(t *testing.T) {
	result := segments.TestSegment("addcid", map[string]string{"filename": "../../../examples/enricher/subnet_ids.csv", "dropunmatched": "true"},
		&pb.EnrichedFlow{RemoteAddr: 0, SrcAddr: []byte{192, 168, 88, 142}})
	if result != nil {
...
func BenchmarkAddCid(b *testing.B) {
	...
	segment := AddCid{}.New(map[string]string{"filename": "../../../examples/enricher/subnet_ids.csv"})
